...
export class QuestionsService {
  ...
  async generateQuestions(): Promise<Question[]> {
    // q1 is always fixed — no need to call Claude for it
    const q1: Question = {
      id: 'q1',
      type: 'open-ended',
...
- Return ONLY the JSON array of 12 objects (q2–q13). No explanation, no markdown.`;
    ...
    try {
      ...
      const generated: Question[] = JSON.parse(cleaned);

      return [q1, ...generated];
    } catch (error) {
      console.error('Error generating questions:', error);
      throw new InternalServerErrorException('Error al generar preguntas');
    }
  }
}
